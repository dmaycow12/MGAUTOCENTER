...
function VendaRowInner({ os, notas = [], clientes = [], onEdit, onDelete, onRefresh, onUpdate, colunas = COLUNAS_PADRAO, ocultarVeiculo = false, rowIndex, getRowRef, registerRef }, ref) {
  ...
  const desvincularNota = async (nota) => {
    const ok = window.confirm(`Desvincular a ${nota.tipo} nº ${nota.numero} desta venda? A nota fiscal não será excluída, apenas desvinculada da venda.`);
    if (!ok) return;
    await base44.entities.NotaFiscal.update(nota.id, { ordem_venda_id: "" });
    onRefresh?.();
  };
