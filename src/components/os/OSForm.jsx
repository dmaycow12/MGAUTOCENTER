...
export default function OSForm({ os, clientes, veiculos, onClose, onSave }) {
  ...
  const gerarLancamentosFinanceiros = async (osData) => {
    ...
    for (const p of parcelas) {
      ...
      const pagoParc = ["Dinheiro", "PIX"].includes(formaParc);
      await base44.entities.Financeiro.create({
        ...
        status: pagoParc ? "Pago" : "Pendente",
        data_pagamento: pagoParc ? new Date().toISOString().split("T")[0] : "",
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black/80 z-50 overflow-y-auto">
      ...
      <form autoComplete="off" onSubmit={e => e.preventDefault()} onKeyDown={e => { if (e.key === "Enter" && e.target.tagName !== "TEXTAREA" && e.target.tagName !== "SELECT") e.preventDefault(); }} className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-3xl my-4">
