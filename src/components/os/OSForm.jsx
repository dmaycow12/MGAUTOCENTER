...
export default function OSForm({ os, clientes, veiculos, onClose, onSave }) {
  ...
  useEffect(() => {
    if (!os) {
      base44.entities.Venda.list("-created_date", 500).then(all => {
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (!os) {
        const todas = await base44.entities.Venda.list("-created_date", 500);
      ...
      if (os) {
        await base44.entities.Venda.update(os.id, formToSave);
      } else {
        const criada = await base44.entities.Venda.create(formToSave);
        savedId = criada.id;
