          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              {["Pendente","Atrasado","Pago","Todos"].map(s => (
                <button key={s} onClick={() => setFiltroStatus(s)} className={`flex-1 h-11 rounded-xl text-sm font-medium transition-all ${filtroStatus === s ? "bg-[#062C9B] text-white" : "bg-gray-800 border border-gray-700 text-gray-400 hover:text-white"}`}>{s}</button>
              ))}
            </div>
