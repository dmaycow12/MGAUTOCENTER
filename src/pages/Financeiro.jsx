          ...
          {filtrados.length === 0 ? (
            ...
            <div className="space-y-4">
              {["Receita", "Despesa"].map(tipo => {
                const itemsTipo = sortedFiltrados.filter(item => item.tipo === tipo);
                if (itemsTipo.length === 0) return null;
                return (
                  <div key={tipo} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    {/* Título centralizado */}
                    <div className="flex items-center justify-center px-4 py-3 border-b border-gray-700 bg-gray-800/50">
                      <h3 className="text-sm font-bold text-white" style={{color: tipo === "Receita" ? "#00ff00" : "#ff0000"}}>
                        {tipo}
                      </h3>
                    </div>
