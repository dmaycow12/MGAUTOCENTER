...
export default function NotasFiscais() {
  ...
  const [confirmModal, setConfirmModal] = useState(null); // { msg, onConfirm, type }
  const showConfirm = (msg, type = 'confirm') => new Promise((resolve) => setConfirmModal({ msg, onConfirm: resolve, type }));
    ...
    if (params.get("emitir") === "1") {
      (async () => {
        ...
        const dadosCliente = c ? {
          ...
          cliente_cep: c.cep || "",
          cliente_cidade: (c.cidade || venda?.cliente_cidade || "").toUpperCase(),
          cliente_estado: (c.estado || venda?.cliente_estado || "").toUpperCase(),
          ...
          cliente_cidade: (venda?.cliente_cidade || "").toUpperCase(),
          cliente_estado: (venda?.cliente_estado || "").toUpperCase(),
          forma_pagamento: formaPagamentoVenda,
          parcelas: venda?.parcelas || 1,
          parcelas_detalhes: parcelasDetalhesNormalizadas,
        };
  ...
  const selecionarCliente = (clienteId) => {
    ...
    setForm(f => ({
      ...
      cliente_cep: c.cep || "",
      cliente_cidade: (c.cidade || "").toUpperCase(),
      cliente_estado: (c.estado || "").toUpperCase(),
    }));
  ...
  const excluir = async (id) => {
    ...
    await showConfirm("Excluir esta nota fiscal?", 'delete');
    setConfirmModal(null);
    if (nota?.numero) {
      const financeiros = await base44.entities.Financeiro.list("-created_date", 500);
  ...
  const emitirNota = async (rascunhoNota = null) => {
    ...
    if (rascunhoNota) {
      ...
      f = {
        ...
        cliente_cidade: (clienteVinculado?.cidade || '').toUpperCase(),
        cliente_estado: (clienteVinculado?.estado || '').toUpperCase(),
  ...
  const editarNota = (nota) => {
    ...
    setForm({
      ...
      cliente_cidade: (nota.cliente_cidade || '').toUpperCase(),
      cliente_estado: (nota.cliente_estado || '').toUpperCase(),
            ...
            <div key={nota.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition-all">
              ...
              <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-white/10">
                ...
                <div className="flex gap-0.5">
                  ...
                  <button title="Abrir PDF (clique do meio abre em nova aba)" onClick={() => abrirPdfNota(nota)} onAuxClick={(e) => { if (e.button === 1) { e.preventDefault(); abrirPdfNota(nota); } }} onMouseDown={(e) => { if (e.button === 1) e.preventDefault(); }} className="w-7 h-7 flex items-center justify-center rounded-lg" style={{ color: nota.pdf_url ? "#00ff00" : "#ef4444" }}><FileText className="w-3.5 h-3.5"/></button>
                  ...
                  <tr key={nota.id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-all">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button title="Abrir PDF (clique do meio abre em nova aba)" onClick={() => abrirPdfNota(nota)} onAuxClick={(e) => { if (e.button === 1) { e.preventDefault(); abrirPdfNota(nota); } }} onMouseDown={(e) => { if (e.button === 1) e.preventDefault(); }} className="p-1 transition-all" style={{ color: nota.pdf_url ? "#00ff00" : "#ef4444" }}>
                    ...
                    {form.items.map((item, idx) => (
                      <div key={idx} className="bg-gray-800/50 rounded-xl p-3">
                        {!item.descricao ? (
                          ...
                          /* NFe/NFCe — layout igual ao de Produtos em Vendas */
                          <div className="space-y-1">
                            ...
                            <div className="hidden lg:flex flex-wrap lg:flex-nowrap gap-2 items-end">
                              ...
                              <div className="flex-1 min-w-[200px]">
                                ...
                                <input value={item.descricao} onChange={e => atualizarItem(idx, 'descricao', e.target.value)} className={`input-dark ${errosForm.items?.[idx]?.descricao ? 'border-red-500' : ''}`} autoComplete="off" />
          ...
          <div className="relative w-full max-w-sm rounded-2xl overflow-hidden border border-gray-700 shadow-2xl" style={{background:'#0d1117'}}>
            <div className="px-6 pt-6 pb-4 flex flex-col items-center gap-3">
              {confirmModal.type === 'delete' ? (
                <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{background:'rgba(239,68,68,0.1)', border:'2px solid #ef4444'}}>
              ...
              <h2 className="text-white font-bold text-lg text-center">
                {confirmModal.type === 'delete' ? 'Confirmar Exclusão' : 'Confirmar Autorização'}
              </h2>
            ...
            <div className="flex gap-3 p-5">
              ...
              <button
                ...
                style={{background: confirmModal.type === 'delete' ? '#ef4444' : '#00ff00', color: confirmModal.type === 'delete' ? '#fff' : '#000'}}
                onMouseEnter={e => e.currentTarget.style.background = confirmModal.type === 'delete' ? '#dc2626' : '#00dd00'}
                onMouseLeave={e => e.currentTarget.style.background = confirmModal.type === 'delete' ? '#ef4444' : '#00ff00'}
              >
                {confirmModal.type === 'delete' ? 'Excluir' : 'Autorizar'}
              </button>
            </div>
          </div>
        </div>
