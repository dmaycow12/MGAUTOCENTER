...
Deno.serve(async (req) => {
  try {
    ...
    if (clientes?.[0]?.nome?.toUpperCase() === "CONSUMIDOR") {
      return Response.json({ criado: false, motivo: 'consumidor' });
    }

    // Verifica se o veículo já existe
    const veiculos = await base44.asServiceRole.entities.Veiculo.list("-created_date", 500);
    ...
    // Cria novo veículo
    const novoVeiculo = await base44.asServiceRole.entities.Veiculo.create({
      cliente_id,
