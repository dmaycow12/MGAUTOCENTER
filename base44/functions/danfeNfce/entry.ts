...
<style>
  ...
  @media print {
    #btn-salvar-pdf { display: none !important; }
  }
</style>
...
Deno.serve(async (req) => {
  try {
    ...
    if (nota.pdf_url && (nota.pdf_url.endsWith('.html') || nota.pdf_url.includes('/notas_fiscais_consumidor/'))) {
      ...
      // Salva chave e html_url se ainda não estava salvo
    ...
    // Injeta botão de salvar PDF antes do </body>
    if (htmlContent.includes('</body>')) {
      htmlContent = htmlContent.replace('</body>', PRINT_BUTTON_HTML + '</body>');
    } else {
      htmlContent = htmlContent + PRINT_BUTTON_HTML;
    }

    return Response.json({ sucesso: true, html: htmlContent });
