    ...
    for (let vi = 0; vi < allVendas.length; vi++) {
      ...
      for (let i = 0; i < parcelasAtualizadas.length; i++) {
        ...
        const fin = await withRetry(() => base44.asServiceRole.entities.Financeiro.create({
          ...
          descricao: `#${venda.numero} — ${venda.cliente_nome_fantasia || venda.cliente_nome || ""}${venda.veiculo_modelo ? ` — ${venda.veiculo_modelo}` : ""}${venda.veiculo_placa ? ` — ${venda.veiculo_placa}` : ""} — ${i+1}/${totalParcelas}`,
